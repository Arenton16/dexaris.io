  ...
  return (
    ...
    >
      <div className="chart-card-header">
        <div>
          <h3 className="chart-title" style={{ fontSize: '12px', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em', color: 'rgba(242,242,242,0.45)', margin: 0 }}>{title}</h3>
          {subtitle && <p style={{ margin: '3px 0 0', fontSize: '12px', color: 'rgba(242,242,242,0.3)', fontWeight: 400 }}>{subtitle}</p>}
        </div>
        <button
...
function ScatterTooltip({ active, payload }: { active?: boolean; payload?: Array<{ payload: ScatterPoint }> }) {
  ...
  const row = (label: string, value: string) => (
    <p style={{ margin: 0, display: 'flex', gap: 6 }}>
      <span style={{ color: 'rgba(242,242,242,0.4)', minWidth: 40 }}>{label}</span>
      <span>{value}</span>
    </p>
  );
  return (
...
function ScoreScatterTooltip({ active, payload }: { active?: boolean; payload?: Array<{ payload: ScoreScatterPoint }> }) {
  ...
  const row = (label: string, value: string) => (
    <p style={{ margin: 0, display: 'flex', gap: 6 }}>
      <span style={{ color: 'rgba(242,242,242,0.4)', minWidth: 50 }}>{label}</span>
      <span>{value}</span>
    </p>
  );
  return (
...
function QuadrantOverlay() {
  ...
  return (
    <g>
      ...
      <line x1={left}       y1={thresholdY} x2={left + w}   y2={thresholdY} stroke="rgba(255,255,255,0.3)" strokeDasharray="4 4" strokeWidth={1} />
...
function ApyScoreQuadrant() {
  ...
  return (
    <g>
      ...
      <line x1={left}       y1={thresholdY} x2={left + w}   y2={thresholdY} stroke="rgba(255,255,255,0.15)" strokeDasharray="4 4" strokeWidth={1} />
      <text x={left + w - 4} y={thresholdY - 4} textAnchor="end" {...ap}>15% APY</text>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '20px', alignItems: 'stretch' }}>
              ...
              >
                ...
                <div style={{ flex: '0 0 30%', display: 'flex', flexDirection: 'column', justifyContent: 'flex-start' }}>
                  <span style={{ fontSize: '11px', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.08em', color: 'rgba(242,242,242,0.35)', display: 'block', marginBottom: '14px' }}>Avg Dexaris Score</span>
                  <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                    ...
                    <span style={{ fontSize: '12px', color: 'rgba(242,242,242,0.3)', marginTop: '10px', display: 'block' }}>Across {insightData.poolCount.toLocaleString()} scored pools</span>
                  </div>
                </div>
                {/* Zone 2 — tier gauge */}
                <div style={{ flex: '1', display: 'flex', flexDirection: 'column', justifyContent: 'flex-start', paddingLeft: '20px', paddingRight: '20px', borderLeft: '1px solid rgba(242,242,242,0.06)' }}>
                  <span style={{ fontSize: '11px', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.08em', color: 'rgba(242,242,242,0.35)', display: 'block', marginBottom: '14px' }}>Score Gauge</span>
                  <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '4px' }}>
                      <span style={{ fontSize: '10px', color: 'rgba(242,242,242,0.3)' }}>0</span>
                      <span style={{ fontSize: '10px', color: 'rgba(242,242,242,0.3)' }}>100</span>
                    </div>
                  </div>
                </div>
                {/* Zone 3 — mini distribution */}
                <div style={{ flex: '1', display: 'flex', flexDirection: 'column', justifyContent: 'flex-start', paddingLeft: '20px', borderLeft: '1px solid rgba(242,242,242,0.06)' }}>
                  <span style={{ fontSize: '11px', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.08em', color: 'rgba(242,242,242,0.35)', display: 'block', marginBottom: '14px' }}>Distribution</span>
                  <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
              ...
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingBottom: '14px', borderBottom: '0.5px solid rgba(242,242,242,0.06)' }}>
                  <div>
                    <span style={{ display: 'block', fontSize: '11px', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.08em', color: 'rgba(242,242,242,0.35)' }}>Average APY</span>
                    <span style={{ fontSize: '12px', color: 'rgba(242,242,242,0.3)' }}>across all pools</span>
                  </div>
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end' }}>
                    <span style={{ fontSize: '22px', fontWeight: 600, color: '#F2F2F2' }}>{insightData.avgApy.toFixed(2)}%</span>
                    {insightData.avgApyDelta !== null && (
                      <span style={{ fontFamily: 'Inter, sans-serif', fontSize: '12px', color: insightData.avgApyDelta >= 0 ? '#4ECDA4' : '#FF6B6B', marginTop: '2px' }}>
                        {insightData.avgApyDelta >= 0 ? '+' : ''}{insightData.avgApyDelta.toFixed(2)}% <span style={{ color: 'rgba(242,242,242,0.3)' }}>24h</span>
                      </span>
                    )}
                  </div>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: '14px', paddingBottom: '14px', borderBottom: '0.5px solid rgba(242,242,242,0.06)' }}>
                  <div>
                    <span style={{ display: 'block', fontSize: '11px', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.08em', color: 'rgba(242,242,242,0.35)' }}>Protocols Tracked</span>
                    <span style={{ fontSize: '12px', color: 'rgba(242,242,242,0.3)' }}>{insightData.chainCount} chains</span>
                  </div>
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end' }}>
                    <span style={{ fontSize: '22px', fontWeight: 600, color: '#F2F2F2' }}>{insightData.protocolCount}</span>
                    <span style={{ fontFamily: 'Inter, sans-serif', fontSize: '12px', color: 'rgba(242,242,242,0.45)', marginTop: '2px' }}>{insightData.strongCount} scoring Strong</span>
                  </div>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: '14px' }}>
                  <div>
                    <span style={{ display: 'block', fontSize: '11px', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.08em', color: 'rgba(242,242,242,0.35)' }}>Best Performing Chain</span>
                  </div>
                  {insightData.bestChain && (
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        ...
                        <span style={{ fontSize: '18px', fontWeight: 600, color: '#F2F2F2' }}>{insightData.bestChain.chain}</span>
                      </div>
          ...
          <ResponsiveContainer width="100%" height={420} style={{ marginLeft: -8 }}>
            <ScatterChart margin={{ top: 4, right: 16, bottom: 24, left: 8 }}>
              ...
              {Object.entries(chainGroups)
                ...
                .map(([chain, points]) => (
                  <Scatter key={chain} name={chain} data={points} fill={SCATTER_CHAIN_COLORS[chain] ?? 'rgba(242,242,242,0.3)'} shape={ScatterDot} />
                ))}
            </ScatterChart>
          </ResponsiveContainer>
            ...
            {legendChains.map(chain => {
              ...
              return (
                <span key={chain} className="scatter-legend-item" onClick={() => toggleChain(chain)}
                  ...
                  <span className="scatter-legend-dot" style={{ background: SCATTER_CHAIN_COLORS[chain] ?? 'rgba(242,242,242,0.3)' }} />
                  {chain}
                </span>
              );
            })}
          ...
          >
            <ResponsiveContainer width="100%" height={380} style={{ marginLeft: -8 }}>
              <BarChart data={topByScoreDeduped} layout="vertical" margin={{ top: 4, right: 56, bottom: 4, left: 0 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="rgba(242,242,242,0.06)" horizontal={false} />
        ...
        >
          <ResponsiveContainer width="100%" height={420} style={{ marginLeft: -8 }}>
            <ScatterChart margin={{ top: 4, right: 16, bottom: 24, left: 8 }}>
              ...
              <ZAxis range={[1, 1]} />
              <ApyScoreQuadrant />
              <Tooltip content={<ScoreScatterTooltip />} wrapperStyle={{ overflow: 'visible', zIndex: 100 }} cursor={{ stroke: 'rgba(242,242,242,0.2)', strokeWidth: 1 }} />
