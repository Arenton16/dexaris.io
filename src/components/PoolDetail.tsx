...
function ChangeLabel({ value }: { value: number | null }) {
  if (value === null) return <span style={{ color: 'rgba(242,242,242,0.3)' }}>—</span>;
        ...
        {!loading && !symbols.some(s => prices[s]) ? (
          ...
          symbols.map(sym => (
            <div key={sym} style={{
              ...
              <div style={{ width: 36, height: 11, borderRadius: 4, background: 'rgba(242,242,242,0.07)', flex: 'none' }} />
              <div style={{ flex: 1, height: 11, borderRadius: 4, background: 'rgba(242,242,242,0.05)' }} />
              <div style={{ width: 56, height: 24, borderRadius: 4, background: 'rgba(242,242,242,0.05)' }} />
            </div>
          ))
        ) : (
          symbols.map(sym => {
            ...
            return (
              <div key={sym} style={{
                ...
                <span style={{ fontSize: 12, fontWeight: 600, color: '#F2F2F2', minWidth: 48, flex: 'none' }}>
                  {sym}
                </span>
                <span style={{ fontSize: 13, fontWeight: 500, color: '#F2F2F2', flex: 1, fontVariantNumeric: 'tabular-nums' }}>
                  {p.price !== null
                    ? p.isStable
                      ? `$${p.price.toFixed(4)}`
                      : p.price >= 1000
                ...
                {p.isStable ? (
                  <span style={{ fontSize: 10, color: 'rgba(242,242,242,0.3)', letterSpacing: '0.06em', textTransform: 'uppercase', flex: 'none' }}>
                    Stablecoin
                  </span>
                ) : (
                  <>
                    <span style={{ fontSize: 11, flex: 'none', minWidth: 52, textAlign: 'right', fontVariantNumeric: 'tabular-nums' }}>
                      <span style={{ fontSize: 9, color: 'rgba(242,242,242,0.3)', display: 'block', marginBottom: 1 }}>24h</span>
                      <ChangeLabel value={p.change24h} />
                    </span>
                    <span style={{ fontSize: 11, flex: 'none', minWidth: 52, textAlign: 'right', fontVariantNumeric: 'tabular-nums' }}>
                      <span style={{ fontSize: 9, color: 'rgba(242,242,242,0.3)', display: 'block', marginBottom: 1 }}>7d</span>
        ...
        {pool && (() => {
          ...
          const breakdownRows = breakdown.components.map(comp => (
            <div key={comp.label} style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
              <span style={{ fontSize: 11, color: 'rgba(242,242,242,0.45)', minWidth: 92, flex: 'none' }}>{comp.label}</span>
              <div style={{ flex: 1, height: 4, background: 'rgba(242,242,242,0.08)', borderRadius: 2, overflow: 'hidden' }}>
                <div style={{ width: `${(comp.score / 10) * 100}%`, height: '100%', background: scoreColour, borderRadius: 2 }} />
              </div>
              <span style={{ fontSize: 11, color: 'rgba(242,242,242,0.45)', minWidth: 36, textAlign: 'right', flex: 'none', fontVariantNumeric: 'tabular-nums' }}>{comp.score.toFixed(1)}/10</span>
            </div>
          ));
          ...
          const scoreSparklineEl = (() => {
            if (scoreHistoryData.length < MIN_SCORE_HISTORY) {
              return (
                <div style={{ marginTop: 14, paddingTop: 12, borderTop: '0.5px solid rgba(242,242,242,0.07)', display: 'flex', alignItems: 'center', gap: 6 }}>
                  <span style={{ fontSize: 10, fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'rgba(242,242,242,0.25)' }}>Score (30d)</span>
                  <span style={{ fontSize: 10, color: 'rgba(242,242,242,0.25)', fontStyle: 'italic' }}>Building score history…</span>
                </div>
              );
            }
            ...
            return (
              <div style={{ marginTop: 14, paddingTop: 12, borderTop: '0.5px solid rgba(242,242,242,0.07)' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
                  <span style={{ fontSize: 10, fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'rgba(242,242,242,0.4)' }}>Score (30d)</span>
          ...
          const yieldCompBody = pool.apyBase == null ? (
            ...
            <>
              ...
              <p style={{ margin: 0, fontSize: 11, color: 'rgba(242,242,242,0.45)' }}>100% organic yield</p>
            </>
          ) : (
            <>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  ...
                  <span style={{ fontSize: 11, color: 'rgba(242,242,242,0.45)' }}>Organic {pool.apyBase.toFixed(2)}%</span>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  <span style={{ width: 8, height: 8, borderRadius: 2, background: '#FFB347', flex: 'none', display: 'inline-block' }} />
                  <span style={{ fontSize: 11, color: 'rgba(242,242,242,0.45)' }}>Incentive {extPool.apyReward.toFixed(2)}%</span>
                </div>
              </div>
            </>
          );
          ...
          const chipStyle = { fontSize: 11, padding: '3px 10px', borderRadius: 20, background: 'rgba(242,242,242,0.06)', border: '0.5px solid rgba(242,242,242,0.15)', color: 'rgba(242,242,242,0.55)' };
          ...
          const yieldSourceEl = (
            <div style={{ background: 'rgba(242,242,242,0.03)', border: '0.5px solid rgba(242,242,242,0.08)', borderRadius: '10px', padding: '14px 16px' }}>
              <span style={SEC_LABEL}>Yield Source</span>
              <span style={{ fontSize: '15px', fontWeight: 600, color: '#F2F2F2', display: 'block', marginBottom: '6px' }}>{yieldSource.label}</span>
              <span style={{ fontSize: '12px', color: 'rgba(242,242,242,0.45)', lineHeight: 1.6, display: 'block' }}>{yieldSource.description}</span>
            </div>
          );
            ...
            return (
              <div className="detail-content" style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                ...
                <div style={{ display: 'flex', gap: '12px', width: '100%' }}>
                  {(() => {
                    const statCard = { flex: 1, background: 'rgba(242,242,242,0.03)', border: '0.5px solid rgba(242,242,242,0.08)', borderRadius: '10px', padding: '14px 16px', display: 'flex', flexDirection: 'column' as const, gap: '6px' };
                    const statLabel = { fontSize: '10px', fontWeight: 400, color: 'rgba(242,242,242,0.4)', textTransform: 'uppercase' as const, letterSpacing: '0.5px' };
                  ...
                  <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '16px' }}>
                    ...
                    <div style={{ ...CARD, flex: 1 }}>
                      ...
                      {(() => {
                        ...
                        const labelStyle = { fontSize: '11px', color: 'rgba(242,242,242,0.4)' };
                        const valueStyle = { fontSize: '11px', color: 'rgba(242,242,242,0.8)', fontWeight: 500 };
                        const divStyle = { height: '0.5px', background: 'rgba(242,242,242,0.06)' };
                        return (
                          <>
                            <div style={rowStyle}>
                ...
                {/* Row 3 — Pool Insight */}
                <div style={{ background: 'rgba(255,255,255,0.07)', border: '0.5px solid rgba(255,255,255,0.2)', borderRadius: '10px', padding: '16px' }}>
                  <span style={{ ...SEC_LABEL, color: 'rgba(255,255,255,0.7)' }}>Pool Insight</span>
                  <p style={{ margin: 0, fontSize: '13px', lineHeight: 1.7, color: 'rgba(242,242,242,0.65)' }}>{insightText}</p>
                  <p style={{ margin: 0, marginTop: '10px', fontSize: '13px', lineHeight: 1.7, color: 'rgba(242,242,242,0.65)' }}>{insightSynthesis}</p>
                </div>
          ...
          return (
            <div className="detail-content">
              ...
              {/* Score Breakdown */}
              <div style={{ margin: '16px 0', background: 'rgba(255,255,255,0.07)', border: '1px solid rgba(255,255,255,0.2)', borderRadius: 10, overflow: 'hidden' }}>
                <div style={{ padding: '10px 14px 8px', borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
                  <span style={{ fontSize: 10, fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'rgba(242,242,242,0.4)' }}>Score Breakdown</span>
                </div>
              ...
              <div style={{ margin: '16px 0', background: 'rgba(255,255,255,0.07)', border: '1px solid rgba(255,255,255,0.2)', borderRadius: 10, overflow: 'hidden' }}>
                <div style={{ padding: '10px 14px 8px', borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
                  <span style={{ fontSize: 10, fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'rgba(242,242,242,0.4)' }}>Yield Composition</span>
                </div>
              ...
              {/* Pool Facts */}
              <div style={{ margin: '16px 0 0', background: 'rgba(255,255,255,0.07)', border: '1px solid rgba(255,255,255,0.2)', borderRadius: 10, overflow: 'hidden' }}>
                <div style={{ padding: '10px 14px 8px', borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
                  <span style={{ fontSize: 10, fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'rgba(242,242,242,0.4)' }}>Pool Facts</span>
                </div>
